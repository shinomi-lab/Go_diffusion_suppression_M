...
func use_DP(adj [][]int, interest_list [][]int,assum_list [][]int, user_weight float64, capacity float64, use_user bool)([][]int, []int, [2][2][2][2]float64, [2]int, [][]int, [][]int){
		...
		fmt.Println("cost_sum:",cost_sum)
		nonF_SeedSet = make([]int, len(adj))//念のため初期化
		DP_ans,_ = opt.DP(0,100,adj,nonF_SeedSet, prob_map,pop_list,interest_list,assum_list,infler_num,true,capacity,max_user,true, user_weight,!use_user,10,non_use_list,use_user)
		fmt.Println("DP_time:",time.Since(s))
